...
export async function recapView(clientId) {
  ...
  const [sessions, memos, plans, recaps, trainerName, recapClosing] = await Promise.all([
  ...
  function generate() {
    ...
    const recap = buildRecap({ client, sessions, from: from.value, to: to.value, nextPlan, settings: { trainerName, recapClosing } });
